...
class Bird(object):
    ...
    def birdUpdate(seif):
        if self.jump:
            ...
            self.birdY -= self.jumpSpeed  # 鸟Y轴坐标减少，小鸟上升
        else:
            # 小鸟坠落
...
def checkDead():
    # 上方管子的矩形位置
    upRect = pygame.Rect(Pipeline.wallx,500
                         Pipeline.pineUp.get_width() -10,
                         Pipeline.pineUp.get_height())

    # 下方管子的矩形位置
    downRect = pygame.Rect(Pipeline.wallx,500,
                           Pipeline.pine.Down.get_width() -10,
                           Pipeline.pine.Down.get_height())
    # 检测小鸟与上下管子是否碰撞
